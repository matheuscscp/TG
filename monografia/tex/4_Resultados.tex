...
A forma que utilizamos para representar fórmulas lógicas em textos teóricos, ou seja, cadeias da respectiva linguagem formal, mostra-se pouco prática para implementar transformações ou algoritmos de busca.
...
Primeiro, observamos os resultados das Combinações 1 e 2, onde não foi feito nenhum renomeamento e alternamos somente entre aplicar ou não simplificação durante a etapa de distribuição. Na Combinação 1, onde simplificação não foi aplicada, 73\% das fórmulas excederam o limite de memória na etapa de pré-processamento e o restante foi transformado com sucesso. Na Combinação 2, 67\% excedeu o limite de memória e 1\% excedeu o limite de tempo. Além disso, pelas medidas das transformações feitas com sucesso em ambas as combinações, nota-se que grande parte das fórmulas ficaram muito menores apenas com simplificação. Isto faz bastante sentido, pois se uma fórmula contém exatamente $n$ símbolos proposicionais distintos, então:
\begin{enumerate}
...
\end{enumerate}
Em outras palavras, uma fórmula com as simplificações que aplicamos possui um tamanho máximo ao ser convertida para a FNC. No entanto, a observação principal é que apenas simplificação não é suficiente para obter bons resultados, já que mais da metade das fórmulas sequer pôde ser pré-processada com sucesso dentro das restrições de tempo e memória utilizadas. Portanto, as próximas comparações que apresentamos levam em conta somente combinações onde renomeamento foi aplicado.

Das Combinações 3 e 5, que aplicam em árvores respectivamente os Algoritmos \ref{boydelatour} e \ref{knapsack} para escolha de renomeamento e não aplicam simplificação durante a etapa de distribuição, verificamos uma amostra de correção da Conjectura \ref{conjectura}. Como dito na Seção \ref{impl} ao descrever a etapa de conversão para FNN, estas combinações simulam árvores lineares. Portanto, pela conjectura, não necessariamente os Algoritmos \ref{boydelatour} e \ref{knapsack} devem encontrar renomeamentos que geram o menor número possível de cláusulas para uma fórmula fixa qualquer, mas ambos devem encontrar renomeamentos que geram exatamente o mesmo número de cláusulas para esta fórmula. Para as fórmulas que puderam ser transformadas com sucesso em ambas as combinações, pudemos constatar este fato.
